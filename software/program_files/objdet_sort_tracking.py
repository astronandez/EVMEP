...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, offset=(0,0)):
    for i, box in enumerate(bbox):
        ...
        id = int(categories[i]) if identities is not None else 0
...
out = cv2.VideoWriter('OutputVideos/outputbikestracked.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 20, (frame_width, frame_height))
...
while True:
    ...
    if ret:
        ...
        result = list(yolo_nas_s.predict(frame, conf=0.35))[0]
        ...
        for (bbox_xyxy, confidence, cls) in zip(bbox_xyxys, confidences, labels):
            ...
            currentArray = np.array([x1, y1, x2, y2, confidence, cls])
